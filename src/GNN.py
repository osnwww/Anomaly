...
class GADAll_GCN(torch.nn.Module):
    ...
    def forward(self, data):
        ...
        for i in range(1):
            A_bar = A @ A.T
        A_bar = A_bar.to_sparse_coo()
        edge_index = A_bar.indices()
        edge_attr = A_bar.values()
...
class GmapAD_SAGE(torch.nn.Module):
    def __init__(self, num_nodes, input_dim, hidden_channels, num_classes):
        super(GmapAD_SAGE, self).__init__()
        torch.manual_seed(7)
        self.name = "GmapAD_SAGE"
        self.conv1 = SAGEConv(input_dim, hidden_channels)
        self.conv2 = SAGEConv(hidden_channels, int(hidden_channels/2))
        self.fc = Linear(int(hidden_channels/2), num_classes)
...
class GmapAD_GIN(torch.nn.Module):
    def __init__(self, num_nodes, input_dim, hidden_channels, num_classes):
        super(GmapAD_GIN, self).__init__()
        torch.manual_seed(7)
        self.name = "GmapAD_GIN"
        self.conv1 = GINConv(torch.nn.Sequential(
            torch.nn.Linear(input_dim,hidden_channels),
            torch.nn.BatchNorm1d(hidden_channels),
            torch.nn. ReLU()))
        self.conv2 =GINConv(torch.nn.Sequential(
            torch.nn.Linear(hidden_channels,int(hidden_channels/2)),
            torch.nn.BatchNorm1d(int(hidden_channels/2)),
            torch.nn. ReLU()))
        self.fc = Linear(int(hidden_channels/2), num_classes)
