...
def align_graphs(graphs: List[np.ndarray],
    ...
    dense_graphs = []
    for graph in graphs:
        adj = to_dense_adj(graph.edge_index)[0].cpu().numpy()
        dense_graphs.append(adj)
    num_nodes = [dense_graphs[i].shape[0] for i in range(len(dense_graphs))]
    ...
    for i in range(len(dense_graphs)):
        num_i = dense_graphs[i].shape[0]

        node_degree = 0.5 * np.sum(dense_graphs[i], axis=0) + 0.5 * np.sum(dense_graphs[i], axis=1)
        ...
        sorted_graph = copy.deepcopy(dense_graphs[i])
...
def graphon_to_graph(graphon):
    label = graphon[0]
    new_graphon = graphon[1]
    graphon_label = torch.from_numpy(label).type(torch.float32)
    new_graph = (np.random.rand(*new_graphon.shape) <= new_graphon).astype(np.int32)
    new_graph = np.triu(new_graph)
    new_graph = new_graph + new_graph.T - np.diag(np.diag(new_graph))

    new_graph = new_graph[new_graph.sum(axis=1) != 0]
    new_graph = new_graph[:, new_graph.sum(axis=0) != 0]

    A = torch.from_numpy(new_graph)
    edge_index, _ = dense_to_sparse(A)

    num_nodes = int(torch.max(edge_index)) + 1

    pyg_graph = Data()
    pyg_graph.y = graphon_label
...
def split_class_graphs(dataset):
    ...
    # c_graph_list = [all_graphs_list[i] for i in range(len(y_list)) if y_list[i] == 0.]
    # class_graphs.append( ( np.array(0), c_graph_list ) )
    # c_graph_list = [all_graphs_list[i] for i in range(len(y_list)) if y_list[i] != 0]
    # class_graphs.append( ( np.array(1), c_graph_list ) )
    return class_graphs
...
def universal_svd(aligned_graphs: List[np.ndarray], threshold: float = 2.02) -> np.ndarray:
    ...
    s[binary_s] = 0 #将s中小于singular_threhold的值置为1
...
def do_edges_split_nc(edges_index, data, val_ratio=0.05, test_ratio=0.1):
    random.seed(234)
    torch.manual_seed(234)
    train_pos_edges = []
    test_pos_edge_indexs = []
    test_neg_edge_indexs = []
    for edge_index,d in zip(edges_index,data):
        ...
        train_pos_edge = torch.cat([train_pos_edge_index, val_pos_edge_index], dim=1)
        train_pos_edges.append(train_pos_edge.t())
        test_pos_edge_indexs.append(test_pos_edge_index.t())
        test_neg_edge_indexs.append(test_neg_edge_index.t())
    return train_pos_edges,test_pos_edge_indexs , test_neg_edge_indexs
...
def prepare_dataset_x(dataset,max_degree):
...
# mask for edge undirected  masking
def edgemask_um(mask_ratio, split_edge, device, num_nodes):
    ...
    pre_index = torch.from_numpy(index[0:-mask_num]).type(torch.long)
    mask_index = torch.from_numpy(index[-mask_num:]).type(torch.long)
...
# mask for edge directed masking
